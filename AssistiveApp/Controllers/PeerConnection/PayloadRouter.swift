...
class PayloadRouter
{
    ...
    var onReceivedNavigationData: ((NavigationDataPayload) -> Void)?
    ...
    private init(){}
        ...
        func handle(payload: Payload){
            ...
            case .navigationData:
                if let dataPayload = try? payload.decode(as: NavigationDataPayload.self){
                    onReceivedNavigationData?(dataPayload)
                }
            }
        }
    }
